...
def find_keyboards():
    ...
    for device in usb.core.find(find_all=True):
        kbd_interface_index, kbd_endpoint_address = adafruit_usb_host_descriptors.find_boot_keyboard_endpoint(device) 
        if isinstance(kbd_interface_index, int) and isinstance(kbd_endpoint_address, int):
            keyboards.append((device, kbd_interface_index, kbd_endpoint_address))
    return keyboards
